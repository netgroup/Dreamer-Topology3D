...
dreamer.DomainController = (function () {
	...
	var self = this;
                $.ajax({
                    ...
                    data: {"topology": this.exportJson(graph), "modelname": this.spec["model_name"]} ,
                    success: function(result) {
                    	var response = {};
                    	response['error'] = false;
           				response = result;
	...
	DomainController.prototype.getRandomTopology = function(n, p, callback){
		        ...
		        $.ajax({
                    url: "http://127.0.0.1:8080/nxbuilder/?n=" + n + "&p=" + p,
	...
	DomainController.prototype.isVisible = function(element, layer){
		if(element instanceof dreamer.Vertex){
			...
			if(this.spec['list_of_all_layer'].indexOf(layername) > -1 
				&& (element.getType() == undefined ||layer_constraints[layername]['list_of_nodes_layer'] === undefined  
					  ||layer_constraints[layername]['list_of_nodes_layer'].indexOf(element.getType()) > -1
					 )
				){
	...
	DomainController.prototype.isValidEdge = function(from_type, to_type, layername){
		var lnae = this.spec['layer_constraints'][layername]['layer_constrints']['not_allowed_edge'];
		...
		if(args.node){
			...
			if(args.graph_parameters.tunneling){
				if(graph.graph_parameters instanceof dreamer.GraphParameters){
					...
					graph.graph_parameters = new dreamer.GraphParameters({tunneling: args.graph_parameters.tunneling});
				}
	...
	DomainController.prototype.import_from_JSON =function(graph, size){
		...
		return {'vertices': nodes, 'edges': edges, 'graph_parameters': graph_parameters};
	}
